...
def main():
    ...
    for sample in range(sample_num):
        for config in range(config_num):
            ...
            conn_en_edges = np.concatenate((conn_en_core_edges, conn_en_pb_edges), dtype=int)
            ...
            for edge in range(m):
                if (b*conn_nu_edges[edge]) < l_edges[edge]:
                    errant_edges += 1
                    print("abelp sample {} config {} edge {} nu = {} < l = {}".format(sample, config, edge, conn_nu_edges[edge], l_edges[edge]))
